...
import examples.VaddStreamReactor


class TestVaddStreamReactor extends AnyFlatSpec with ChiselScalatestTester {

  behavior of "VaddReactor"

  it should "initialize" in {
    test(new TesterWrapper({p => new VaddStreamReactor(p)}, "_test")) { c =>
      c.expectReg("cycles", 0.U)
    }
  }

  it should "perform addition" in {
    test(new TesterWrapper({ p => new VaddStreamReactor(p, vLen = 5) }, "_test")) { c =>
      val baseAddr = 0
      val baseAddrRes = 512
