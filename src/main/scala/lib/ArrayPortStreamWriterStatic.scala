...
class ArrayPortStreamWriter[T <: Data](port: EventArrayWriteMaster[T]) extends Module {}

class ArrayPortStreamWriterIO[T <: Data](genData: T, genToken: ArrayToken[T]) extends Bundle {
  val _port = new EventArrayWriteMaster(genData, genToken)
...
class ArrayPortStreamWriterStatic[T <: Data](port: EventArrayWriteMaster[T], writeAddrSeq: Seq[Int]) extends ArrayPortStreamWriter(port){
...
object ArrayPortStreamWriterStatic {
  def apply[T <: Data](port: EventArrayWriteMaster[T], writeSeq: Seq[Int]) : ArrayPortStreamWriterStaticIO[T] = {
