...
class TopReactorEx extends CodesignMainReactor(TesterWrapperParams) {
  ...
  // Connections
  val c_in = new SingleValueConnectionFactory(defData)
  c_in.addUpstream(in.io.out)
  c_in.addDownstream(c1.io.in)
  c_in.construct()

  val c_c1_c2 = new SingleValueConnectionFactory(
    defData
  )
  c_c1_c2 << c1.io.out1
  c_c1_c2 >> c2.io.in
  ...
  val c_c1_c3 = new SingleValueConnectionFactory(defData)
  c_c1_c3 << c1.io.out2
  c_c1_c3 >> c3.io.in
  c_c1_c3.construct()

  val c_out1 = new SingleValueConnectionFactory(defData)
  c_out1 << c2.io.out

  c_out1 >> out1.io.in
  c_out1.construct()

  val c_out2 = new SingleValueConnectionFactory(defData)
  c_out2 << c3.io.out
  c_out2 >> out2.io.in
  c_out2.construct()
}
